...
| `constructive-functions` | Customer-authored functions & trigger attachments — `db.function` (sql/plpgsql/trigger kinds, validated `bodyAst`, `apiExposed`), `db.trigger` attachments (`events`, `whenAst`) |
...
| `orm-<target>`, `cli-<target>`, `hooks-<target>` | [constructive-io/constructive-db](https://github.com/constructive-io/constructive-db) | Generated per-API-target ORM / CLI / React Query hook references (`default`, `api`, `auth`, `admin`, `compute`, `config`, `infra`, `modules`, `objects`, `storage`, `usage`, `remote`, `migrate`, `agent`, `common`) — regenerated by codegen, never hand-edited |
| `sdk-scope-syntax` | [constructive-io/constructive-db](https://github.com/constructive-io/constructive-db) | Tenant (`db.site`) vs on-prem/platform (`db.platformSite`) lane naming in the ORM |
| `compute-dev-loop`, `compute-sync-functions`, `compute-async-jobs`, `spinning-up` | [constructive-io/constructive-db](https://github.com/constructive-io/constructive-db) | Local compute stack — `fun` CLI, function handlers, sync gateway, app_jobs worker, webhooks |
| `resources-installations` | [constructive-io/constructive-db](https://github.com/constructive-io/constructive-db) | Resource definitions vs installations vs live resources; reconciler projection into Kubernetes |
| `constructive-db-*` | [constructive-io/constructive-db](https://github.com/constructive-io/constructive-db) | SQL-level internals twins of the skills here (`-security`, `-storage`, `-jobs`, `-history`, `-i18n`, `-notifications`, `-principals`, `-billing-limits`, `-data`, `-modules`, `-search`, `-errors`, `-secrets`, `-access-modes`, `-local-k8s`, `-testing`, `-codegen`, `-glossary`) |
