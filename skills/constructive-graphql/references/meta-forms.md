...
export type MetaTable = {
  ...
  query: {
    ...
    one: string | null;  // may be a non-existent root field -- see section 3 bug note
    create: string | null;
    update: string | null;
    delete: string | null;
  };
...
## 3. Platform bug: `query.one` returns a non-existent root field
...
Handles all pgTypes automatically. Add `locked` + `lockedLabel` for pre-filled FK context (see section 8).
...
export type DynamicFormCardProps = {
   ...
   * These fields are rendered as visible-but-locked (disabled, lock icon).
   */
...
// ... (full DynamicFormCard and ConfirmDeleteCard implementation)
// See the full source in this reference for the complete component code.
```
...
**How it renders:**
- `Contact Id` field -> disabled input showing "Kristopher Floyd" + lock icon
