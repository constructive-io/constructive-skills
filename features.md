...
Native PostgreSQL identity columns — `GENERATED ALWAYS AS IDENTITY` and `GENERATED BY DEFAULT AS IDENTITY` — with optional sequence tuning (`START` / `INCREMENT` / `MINVALUE` / `MAXVALUE` / `CACHE` / `CYCLE`). Configured per field via `identityGeneration` + `identityOptions`; mutually exclusive with defaults and generated columns, and hidden from the GraphQL create input.
