...
**Body:**
1. Calls `level_achieved(actor_id)` (or `level_achieved(actor_id, entity_id)`)
2. `level_achieved()` checks all `level_requirements` against `event_aggregates` counts
3. For each newly achieved level, calls `grant_achievement(level_name, actor_id)`
4. `grant_achievement()` is idempotent — unique constraint on `level_grants` prevents re-grants

## tg_achievement_reward

**Created by:** `events_module` provisioning (cross-module wiring with `limits_module`)
**Fires:** AFTER INSERT on `{prefix}_level_grants`
**Security:** SECURITY DEFINER
**Condition:** Only generated when a `limits_module` exists for the same entity scope (same `database_id` + `membership_type`) with a `limit_credits_table_id`.
**Body:**
1. FOR loop over `achievement_rewards WHERE level_name = NEW.level_name`
2. For each reward row:
...
**Why SECURITY DEFINER:** Users don't have INSERT permission on `limit_credits` or `default_limits`. The trigger runs as the database owner to bypass RLS.
...
| `{prefix}_event_aggregates` | Running counts per user per event type (the "achievements" aggregate) |
| `{prefix}_events` | Partitioned event log (time-based partitions via pg_partman) |
| `{prefix}_event_types` | Event type catalog (registered types) |
...
| `{prefix}_level_grants` | Records of which users have earned which levels |
