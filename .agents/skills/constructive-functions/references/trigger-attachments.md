...
Inside the trigger function, `NEW`/`OLD` and `TG_*` behave as in PostgreSQL; the function is always emitted `SECURITY INVOKER`, so the body runs with the privileges and RLS of the role and session performing the write (a `SECURITY DEFINER` function is rejected with `FUNCTION_ENVELOPE_VIOLATION`). To do asynchronous work from a row change, prefer `JobTrigger` (`constructive-jobs`) over calling out from a trigger body.
