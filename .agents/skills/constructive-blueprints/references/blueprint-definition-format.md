...
**Referencing a table by module:** policies that authorize against *another* table (`AuthzValueExists`, `AuthzValueMatch`) identify it with `ref_schema` + `ref_table`, with `ref_table` alone for a sibling table of the same blueprint, or with a `ref_module` [module reference](#referencing-a-module-generated-table) when the table was generated by an installed module:
...
```json
{
  ...
  "privileges": ["select"],
  "data": {
    "ref_module": { "type": "agent", "scope": "app", "table": "thread" },
    "join": [{ "local_column": "thread_ref", "ref_column": "id" }]
  }
}
```

The reference resolves during construction — so a policy can gate your rows on a module's rows without hardcoding the name that install chose, and an unresolvable reference fails the construction rather than authorizing against the wrong table.
...
| `source_module` | object | Yes* | Module reference identifying the source table instead of naming it (see below) |
| `target_module` | object | Yes* | Module reference identifying the target table instead of naming it (see below) |
...
\* Each end is given exactly one way: `source_table` **or** `source_module`, `target_table` **or** `target_module`. Supplying both for the same end is rejected at authoring time.

### Relating to a module-generated table

`source_module` / `target_module` take the same [module reference](#referencing-a-module-generated-table) a table entry takes, so a relation can reach a table an installed module generated without the blueprint knowing the name or schema that install chose — and, unlike the name form, a reference that matches nothing fails the construction instead of quietly creating an empty table to point at.

**A foreign key from your table to a module's table** — "this summary came from that conversation":

```json
{
  "$type": "RelationBelongsTo",
  "source_table": "documents",
  "target_module": { "type": "agent", "scope": "app", "table": "thread" },
  "field_name": "source_thread_id",
  "is_required": false,
  "delete_action": "SET NULL"
}
```

**A junction between your table and a module's table** — pin uploaded files to your own topics:

```json
{
  "$type": "RelationManyToMany",
  "source_table": "topics",
  "target_module": { "type": "storage", "scope": "app", "table": "files" },
  "junction_table_name": "topic_files",
  "data": {
    "nodes": [{ "$type": "DataId", "data": {} }, { "$type": "DataRealtime", "data": {} }]
  }
}
```

**Both ends generated** — wire one module's rows to another's, which no name form can express cleanly since neither name is the blueprint's to know:

```json
{
  "$type": "RelationBelongsTo",
  "source_module": { "type": "agent", "scope": "app", "table": "resource" },
  "target_module": { "type": "storage", "scope": "app", "table": "files" },
  "field_name": "file_id",
  "is_required": false,
  "delete_action": "SET NULL"
