...
| `has_multiple_profiles` | boolean | No | `false` | Let one membership hold several profiles at once, masks unioned. Requires `has_profiles`. See [`constructive-access-control` → profiles.md](../../constructive-access-control/references/profiles.md#multiple-profiles-per-member) |
