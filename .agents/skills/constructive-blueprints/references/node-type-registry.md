...
| `DataGenerated` | Native PostgreSQL generated column computed from a source expression — `GENERATED ALWAYS AS (expr) STORED` (default) or `... VIRTUAL` (PG18+) via `generation_type` |
