...
| `updateUser` returns **HTTP 200 but does not persist** (silent no-op; 200-but-0-rows on the `users` table) | The per-tenant `users` table is missing the `authenticated` **UPDATE** self-policy (`auth_upd_self_update`) — the platform emits it natively now (GAP-1a, CLOSED), so this only hits a deployment predating that fix | → gotchas **`RLS-USERS-UPDATE-001`** · troubleshooting **`## Phase 3: updateUser returns 200 but does not persist`** · fix (predates-platform-fix only): re-provision, or the manual policy in SKILL.md S2 step 3 |
| **Org-scoped INSERT / member write RLS-rejected** for a fresh signup on a **b2b** tenant (200-but-0-rows on an org/`AuthzEntityMembership` table) | The personal-org `org_memberships_sprt` seed / org grants / `create_entity` bit were missing — the platform now provisions all three on signup (GAP-1b/1c, CLOSED), so a denial here is a real regression or a deployment predating the fix | → gotchas **`RLS-ORG-RECONCILE-001`** · `platform-gaps.md` **GAP-1b / GAP-1c** · fix: pull current platform + re-provision with the b2b preset |
