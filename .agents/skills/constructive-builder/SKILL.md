...
- **S7.5 — Art-direct the SHELL + page COMPOSITION** (optional, default auto-propose). You may **restructure** the app shell (`app-shell.tsx` frame + `hideSidebar`) and each entity page's `return(...)` composition (list → data-table / gallery / split-pane / editorial / board) to express the `design.md`, as long as you **preserve the functional contract** — every `<entity>-*` testid, row-scoping, the hooks/selection/refetch/Stack-pushes, the RLS scoping const(s), and the route/block/flow surfaces. Selectors are testid/role-only, so the gates can't see a restyle. Record the chosen shell/composition/density in the `design.md` `art_direction` block so re-runs stay consistent. → **[`references/art-direction.md`](./references/art-direction.md)** (the rules + the preserve-contract checklist).
...
| [art-direction.md](./references/art-direction.md) | Restructuring the SHELL + page COMPOSITION (guidance only): the two-layer model, the preserve-contract checklist, dials→structure, the archetype palette, the edit seams, the `design.md` `art_direction` record | Re-laying-out the shell/pages (S7.5) — auto-propose structure while preserving the functional contract |
