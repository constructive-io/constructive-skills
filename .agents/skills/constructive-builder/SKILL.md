...
- **S6.5 — Apply the design theme** (after the Blocks `@import`, before the CRUD body). `node scripts/wire-design.mjs --app <app>` derives/compiles the app's `design.md` **faithfully** → writes the shadcn token-override block into `globals.css` (custom tokens/fonts pass through; + optional `next/font` / `defaultTheme` swap), restyling the template **and** every installed Block at once. The compiler is **faithful + advisory** (emits what the `design.md` says — no clamps; it only *warns* on low contrast / >1 accent / AI-purple, and synthesizes any shadcn name you leave unspecified so Rail 2 holds). **Default = auto-propose** a rich, domain-fitting `design.md`; `design: { preset: constructive }` ⇒ **no-op**, today's look preserved. Idempotent + `--dry-run`-able. → `references/design-system.md`.
...
- **S7.5 — AUTHOR the presentation from the `design.md`** (always — this is where the app gets its design). The `design.md` is the full design spec; AUTHOR the whole frontend faithfully from it — customize/replace the stock shadcn components, set the type (fonts/scale/weights), compose the layout (shell + each entity page's `return(...)`: list → data-table / gallery / split-pane / editorial / board), establish real hierarchy + spacing rhythm + intentional ornament, add subtle (reduced-motion) motion. Blocks compose as **ingredients** (restyle them via the token names). The ONLY hard rails are **Rail 1** (the functional contract — every `<entity>-*` testid, row-scoping, the hooks/selection/refetch/Stack-pushes, the RLS scoping const(s), the route/block/flow surfaces) and **Rail 2** (the shadcn token names + Tailwind-v4 wiring, so Blocks render). Selectors are testid/role-only and the token contract is name-only, so the gates can't see your restyle. Record the chosen shell/composition/density in the `design.md` `art_direction` block so re-runs stay consistent. → **[`references/art-direction.md`](./references/art-direction.md)** (the authoring playbook + both preserve-contracts + the taste self-check).
...
| [art-direction.md](./references/art-direction.md) | The AUTHORING PLAYBOOK (guidance only): scaffold the skeleton then AUTHOR the presentation from the `design.md` — taste-skill applied to apps (customize stock components, distinctive type, layout/hierarchy/spacing/ornament, subtle motion, anti-slop bans as JUDGMENT), Rail 1 (functional) + Rail 2 (shadcn-token) preserve-contracts, the archetype palette, the edit seams, the `art_direction` record, the taste self-check | Authoring the frontend (S7.5) — turn the skeleton into a designed app while preserving both rails |
