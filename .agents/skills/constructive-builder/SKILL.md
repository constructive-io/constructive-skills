...
> Picking flows is part of authoring the brief: map each capability ("sign in", "reset password", "manage org members") to a **flow id** in `references/flows.json`, and provision the **union** of those flows' `backend.modules[]` — never `['all']`. The b2b tier additionally needs `scripts/fix-org-grants.sh` until the upstream provisioner gap closes (`references/platform-gaps.md` GAP-1b/1c). See `references/brief-grammar.md` and `references/flow-catalog.md`.
...
| [phase-2-data-model.md](./references/phase-2-data-model.md) | Workspace + provision pkg + blueprint; object-form grants, policy intents, module list, grant reconcile | Provisioning the data model / RLS, or a 2.1/2.3 gate fails |
