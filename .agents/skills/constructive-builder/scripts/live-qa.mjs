...
const step = (s) => console.log(`    ${C.dim('·')} ${s}`);
...
const BASE_URL = (process.env.LIVE_QA_BASE_URL || process.env.BASE_URL || '').replace(/\/+$/, '');
const AB = process.env.LIVE_QA_BROWSER_BIN || 'agent-browser';
...
const AB_TIMEOUT = process.env.LIVE_QA_AB_TIMEOUT || '30000';
...
function setExpectedOrigin(baseUrl) {
  EXPECTED_ORIGIN = originOf(baseUrl);
  return EXPECTED_ORIGIN;
}
function originAssertEnabled() {
...
function isolateBrowserSession() {
  try {
...
function assertActiveTabOrigin(where = 'drive') {
...
const IS_INTERACTABLE_JS = (EL) => `(function(__el){
...
function waitTestid(id, { timeoutMs = 20000, what = id } = {}) {
...
function clickTestidVerify(id, verify, { tries = 3, settleMs = 600 } = {}) {
  const fireNative = `
    var el = ${Q_TESTID(id)};
    if (!el) return 'missing';
...
function testidContainsText(id, text) {
  const js = `
...
function testidContainsTextAny(prefixRe, text) {
  const js = `
...
const ROW_AFFORDANCE_RESOLVE_JS = (affordanceTestid, rowTestid, rowText) => `
...
function clickRowAffordance(affordanceTestid, rowTestid, rowText) {
...
function clickRowAffordanceVerify(affordanceTestid, rowTestid, rowText, verify, { tries = 3, settleMs = 600 } = {}) {
...
async function gqlAuthed(endpoint, query, variables = {}) {
...
async function pollMailpit({ to, subjectIncludes, sinceMs = 0, timeoutMs = 20000 } = {}) {
...
async function ensureSignedIn(ctx, { fresh = false } = {}) {
  const t = authTestids();
...
async function signInWith(ctx, email, password) {
  const t = authTestids();
  try {
...
async function signUpSecondActor(ctx) {
...
// The helpers below are byte-for-byte copies of that scaffolder's inflection
// (words/kebab/camel/pluralizeWords/singularFromTable) + table-match (tableFor) so the
// derived prefix is identical. We copy rather than import because scaffold-frontend.mjs
// runs main() at import time (it is a CLI), which we must not trigger here.
...
/** English-pluralize the LAST word, preserving the rest (blog-post → blog-posts). */
...
async function main() {
  ...
  log(`${C.dim('isolate')} closed prior agent-browser sessions; pinned app origin ${C.bold(EXPECTED_ORIGIN || ctx.baseUrl)}${originAssertEnabled() ? '' : C.yellow(' (origin guard disabled)')}`);
