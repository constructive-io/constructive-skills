---
name: constructive-troubleshooting
description: "Constructive platform failure recipes: exact error strings mapped to root causes and fixes across provisioning, blueprints, RLS, auth, codegen, and frontend. Load when a build step or verify fails."
metadata:
...
> **Tip:** know the exact error string but not the phase? Use the `constructive-error-index` skill where available — a flat `symptom string → cause → fix pointer` lookup (gotcha CODE / this guide's section) that routes you straight to the authoritative entry.
